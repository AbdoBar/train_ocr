...
num_classes = len(tokenizer.word_index) + 1  # +1 لأن الفهرس يبدأ من 1
...
train_input_len = np.ones((len(X_train), 1)) * (IMG_WIDTH // 4)
...
def decode_prediction(pred):
    ...
    texts = tokenizer.sequences_to_texts(pred)
    return texts

# تجربة على صورة واحدة
